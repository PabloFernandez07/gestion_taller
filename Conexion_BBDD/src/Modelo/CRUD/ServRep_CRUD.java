...
public class ServRep_CRUD {
     ...
     * @return
     */
    public static boolean insertarSr(int fkRepara, int fkMecanico, int tiempo) {

        try {

            String sql = "insert into servicio_repara(pkSr, fkRepara, fkMecanico, tiempo) values (?,?,?,?);";
            int pk = pkSrMayor()+ 1;

            try (PreparedStatement ps = Conexion_bbdd.getConnection().prepareStatement(sql)) {
                ps.setInt(1, pk);
                ps.setInt(2, fkRepara);
                ps.setInt(3, fkMecanico);
                ps.setInt(3, tiempo);
    ...
    /**
     * Metodo para encontrar el Id de Servicio/Repara mas alto para al insertar sumar 1
     * a este.
     *
     * @return
     */
