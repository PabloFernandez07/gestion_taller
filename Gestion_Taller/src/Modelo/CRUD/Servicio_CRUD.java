...
public class Servicio_CRUD {
    ...
    public static ArrayList<Servicio> mostrarServiciosMatricula(Connection conn, String matricula) {
        ArrayList<Servicio> listaServ = new ArrayList<>();
        try {
            String sql = "SELECT pkServicio, nombre, precio FROM servicio s \n"
                    + "INNER JOIN servicio_repara sr ON sr.fkServicio = s.pkServicio\n"
                    + "INNER JOIN repara r ON r.pkRepara = sr.fkRepara\n"
                    + "INNER JOIN vehiculo v ON v.pkVehiculo = r.fkVehiculo\n"
                    + "WHERE v.matricula = ?;";
            ...
            while (rs.next()) {
                int pkServicio = rs.getInt("pkServicio");
                String nombre = rs.getString("nombre");
                float precio = rs.getFloat("precio");

                Servicio sr = new Servicio(pkServicio, nombre, precio);
                listaServ.add(sr);
            }
